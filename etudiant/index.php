...
if(isset($_POST["valider"])){
    extract($_POST);
	//On recupere la filère
	$resq1 = "SELECT filiere.Libelle AS nom FROM filiere INNER JOIN classe ON filiere.Code_fil = classe.Code_fil INNER JOIN etudiant ON classe.Code_cl = etudiant.Code_cl WHERE matricule = '$matricule'";
    $resultat1 = $idcnx->query($resq1); 
    

    $resq = "SELECT * FROM etudiant WHERE Matricule='$matricule' AND Mdp = '$password'";
    $resultat = $idcnx->query($resq);
    
    if($resultat != null && $resultat->rowCount()>0){
	  $ligne1 = $resultat1->fetch (PDO::FETCH_ASSOC);
	  $ligne2 = $resultat->fetch (PDO::FETCH_ASSOC);
      session_start();
      $_SESSION['matricule'] = $matricule;
      $_SESSION['Mdp'] = $password;
      $_SESSION['nom'] =$ligne2['Nom'];
	  $_SESSION['prenom'] =$ligne2['Prenom'];
	  $_SESSION['filiere'] =$ligne1['nom'];
